...
export async function validateMCPProject() {
  try {
    ...
    if (
      !package_json.dependencies?.['@magnolia-solutions/mcp-framework'] &&
      !package_json.devDependencies?.['@magnolia-solutions/mcp-framework']
    ) {
      throw new Error(
        'This directory is not an MCP project (@magnolia-solutions/mcp-framework not found in dependencies or devDependencies)'
      );
    }
  } catch (error) {
